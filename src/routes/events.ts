...
router.get("/:id/analytics", requireOrganizer, async (req: Request, res: Response): Promise<void> => {
  const { organizerId } = req as AuthedRequest;
...
// GET /events/analytics/summary — cross-event analytics
router.get("/analytics/summary", requireOrganizer, async (req: Request, res: Response): Promise<void> => {
  const { organizerId } = req as AuthedRequest;
